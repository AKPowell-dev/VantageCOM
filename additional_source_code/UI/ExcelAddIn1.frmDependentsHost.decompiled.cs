...
[DesignerGenerated]
public sealed class frmDependentsHost : TraceForm
{
		...
		if (m.Msg == 28)
		{
			...
			if (m.WParam.ToInt32() == 0)
			{
				...
				if (!IgnoreDeactivate)
				{
					...
					((wpfDependents)ElementHost1.Child).Close();
				}
			}
		}
		base.WndProc(ref m);
	...
	private void A(object A, EventArgs B)
	{
		try
		{
			ElementHost1.Child = new wpfDependents(this);
