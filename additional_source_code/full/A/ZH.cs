...
internal sealed class ZH
{
	public sealed class YH
	{
		...
		static YH()
		{
			YH.m_A = new OpCode[256];
			YH.m_B = new OpCode[256];
		...
		public YH(MethodBase A, byte[] B, DynamicILInfo C)
		{
			this.m_A = C;
			this.m_A = B;
			this.m_A = 0;
			this.m_A = A.Module;
			this.m_A = ((A is ConstructorInfo) ? null : A.GetGenericArguments());
			object b;
	...
	static ZH()
	{
		if (ZH.A == null)
		{
			...
			string s = "TWFjYWJhY3VzLkV4Y2VsKg==";
			...
			ZH.A = UH.A(0, manifestResourceStream);
			ZH.A = new Dictionary<int, int>();
			using BinaryReader binaryReader = new BinaryReader(new MemoryStream(ZH.A, writable: false));
		...
		while (true)
		{
			...
			ZH.m_A = Assembly.GetExecutingAssembly().GetModules()[0].ModuleHandle;
			return;
		}
	}
	...
	internal static void A(int A, int B, int C)
	{
		...
		try
		{
			typeFromHandle = Type.GetTypeFromHandle(ZH.m_A.ResolveTypeHandle(A));
			object methodFromHandle = MethodBase.GetMethodFromHandle(ZH.m_A.ResolveMethodHandle(B), ZH.m_A.ResolveTypeHandle(C));
		...
		foreach (FieldInfo fieldInfo in fields)
		{
			try
			{
				...
				Type[] parameterTypes = ZH.A(methodBase);
				...
				ZH.A.TryGetValue(A, out var value);
				DynamicILInfo dynamicILInfo = dynamicMethod.GetDynamicILInfo();
				ZH.A(methodBody, dynamicILInfo);
				ZH.A(ref value, methodBase, dynamicILInfo);
				ZH.A(ref value, dynamicILInfo);
	...
	{
		int maxStackSize = BitConverter.ToInt32(ZH.A, A);
		A += 4;
		int num = BitConverter.ToInt32(ZH.A, A);
		A += 4;
		byte[] array = new byte[num];
		Buffer.BlockCopy(ZH.A, A, array, 0, num);
		YH yH = new YH(B, array, C);
		yH.A();
	...
	private static void A(ref int A, DynamicILInfo B)
	{
		...
		for (int i = 0; i < num3; i++)
		{
			...
			{
				RuntimeTypeHandle type = ZH.m_A.ResolveTypeHandle(BitConverter.ToInt32(array, num2));
				int tokenFor = B.GetTokenFor(type);
				ZH.A(tokenFor, num2, array);
				break;
			}
