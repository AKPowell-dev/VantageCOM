...
public sealed class PriceAnnotate
{
	private struct OE
	{
		public float A;
	...
	private static readonly string m_A = VH.A(82304);
			...
			while (true)
			{
				...
				if (!Workbooks.IsShared(application.ActiveWorkbook, true, (System.Windows.Window)null))
				{
					...
					try
					{
						...
						ChartObject chartObject = QuickCharts.AddChart(worksheet, oE.A, oE.B);
						...
						while (true)
						{
							...
							((ChartGroup)chart.LineGroups(1)).GapWidth = (int)Math.Round(oE.C);
