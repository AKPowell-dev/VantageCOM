...
{
	public static readonly string PANE_TITLE = AH.A(75085);

	internal static readonly string A = AH.A(75110);
	...
	public static void Toggle(bool blnPressed)
	{
		...
		if (blnPressed)
		{
			...
			if (!flag)
			{
				ctpSelectMatch control = new ctpSelectMatch();
	...
	private static void A(object A, EventArgs B)
	{
		...
		wpfSelectMatch wpfSelectMatch2 = Pane.A((ctpSelectMatch)customTaskPane.Control);
		...
		else if (!KG.A)
		{
			...
			wpfSelectMatch2.HidePane();
			PB.Settings.TaskPaneWidth = checked((int)Math.Round((double)customTaskPane.Width / Dpi.X));
			Pane.A();
		}
		customTaskPane = null;
		wpfSelectMatch2 = null;
	}

	private static wpfSelectMatch A(ctpSelectMatch A)
	{
		return (wpfSelectMatch)A.ElementHost1.Child;
	}
