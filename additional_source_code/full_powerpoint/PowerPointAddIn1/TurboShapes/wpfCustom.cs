...
[DesignerGenerated]
public sealed class wpfCustom : Window, IComponentConnector
{
	...
	private void A()
	{
		...
		{
			currentState = checked(currentState - 1);
		}
		A(currentState);
	}
	...
	{
		int currentState = CurrentState;
		currentState = ((currentState == MaxState) ? 1 : checked(currentState + 1));
		A(currentState);
	}
	...
	private void A(int A)
	{
		...
		try
		{
			...
			Custom.ApplyState(EditedShape, A);
			CurrentState = A;
		}
		catch (Exception ex)
		{
	...
	public void InitializeComponent()
	{
		...
		while (true)
		{
			...
			this.m_A = true;
			Uri resourceLocator = new Uri(AH.A(159549), UriKind.Relative);
