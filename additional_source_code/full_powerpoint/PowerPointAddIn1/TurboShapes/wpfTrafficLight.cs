...
[DesignerGenerated]
public sealed class wpfTrafficLight : Window, IComponentConnector
{
	public Shape EditedShape;

	public TrafficLight.Style LightStyle;

	public List<TrafficLight.TrafficLightState> ShapeStates;

	public TrafficLight.TrafficLightState CurrentState;
	...
	{
		if (LightStyle == TrafficLight.Style.Donut)
		{
			LightStyle = TrafficLight.Style.Solid;
		}
		else
		{
			LightStyle = TrafficLight.Style.Donut;
		}
		A(CurrentState);
	}
	...
	private void A()
	{
		int num = ShapeStates.IndexOf(CurrentState);
		num = checked((num != 0) ? (num - 1) : (ShapeStates.Count - 1));
		A(ShapeStates[num]);
	}
	...
	private void A(TrafficLight.TrafficLightState A)
	{
		...
		try
		{
			...
			shape = TrafficLight.Create(slide, (int)A, (int)LightStyle);
	...
	{
		Key key = e.Key;
		if (key != Key.Return && key != Key.Escape)
		{
			while (true)
			{
				switch (7)
	...
	public void InitializeComponent()
	{
		...
		while (true)
		{
			...
			this.m_A = true;
			Uri resourceLocator = new Uri(AH.A(160787), UriKind.Relative);
