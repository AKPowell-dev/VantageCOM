...
public sealed class MissingBraces : BaseTextCheck
{
	...
	{
		base.RegexObj = new Regex(XC.A(25190));
		A = new Regex(XC.A(25215));
		B = new Regex(XC.A(25220));
	}
		...
		checked
		{
			...
			try
			{
				...
				while (enumerator2.MoveNext())
				{
					...
					Main.Analysis.Errors.Add(new MissingClosingCurlyBrace(duplicate));
					duplicate = null;
				}
				while (true)
				{
			...
			try
			{
				...
				while (enumerator3.MoveNext())
				{
					...
					Main.Analysis.Errors.Add(new MissingOpeningCurlyBrace(duplicate2));
					duplicate2 = null;
				}
				while (true)
				{
	...
	public override void Check(object shp, TextRange2 rng, string strText)
	{
		...
		checked
		{
			try
			{
				...
				{
					Match match2 = (Match)enumerator2.Current;
					Main.Analysis.Errors.Add(new MissingClosingCurlyBrace(RuntimeHelpers.GetObjectValue(shp), rng.get_Characters(match2.Index + 1, match2.Length)));
				}
				while (true)
				{
					switch (7)
			...
			try
			{
				...
				{
					Match match3 = (Match)enumerator3.Current;
					Main.Analysis.Errors.Add(new MissingOpeningCurlyBrace(RuntimeHelpers.GetObjectValue(shp), rng.get_Characters(match3.Index + 1, match3.Length)));
				}
				while (true)
				{
					switch (1)
